...
async function linkProducts() {
  ...
  try {
    const userRes = await client.query(`SELECT id FROM public.profiles WHERE email ILIKE '%mariano260996%';`);
    ...
    const companyRes = await client.query(`SELECT id, name FROM public.companies WHERE email ILIKE '%mariano260996%' OR name ILIKE '%ElleonStore%';`);
    ...
    console.log(`Vincular productos del usuario ${userId} a la empresa "${companyName}" (${companyId})...`);
    ...
    console.log(`✓ Se vincularon ${updateRes.rowCount} productos a la empresa "${companyName}"!`);
  } catch (err) {
