...
async function main() {
  ...
  try {
    const resProfiles = await client.query(`SELECT * FROM public.profiles WHERE email ILIKE '%mariano260996%';`);
    ...
    const resMembers = await client.query(`SELECT * FROM public.company_members;`);
