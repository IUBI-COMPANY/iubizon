    ...
    <div className="min-h-screen flex flex-col bg-[#f8fafc]">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#64748b] mt-0.5">
              Consulta el resumen de tus compras y haz clic en cualquiera para ver el seguimiento detallado.
            </p>
          </div>
        ...
        {!loading && sessions.length > 0 && (() => {
          ...
          const filteredSessions = statusTab === "all"
            ? sessions
            : statusTab === "in_transit"
              ? sessions.filter((s) => s.packages.some((p) => p.status === "shipped" || p.status === "paid"))
              : statusTab === "pending"
                ? sessions.filter((s) => s.packages.every((p) => p.status === "pending"))
                : sessions.filter((s) => s.packages.every((p) => p.status === "delivered" || p.status === "completed"));
            ...
            <>
              ...
              {filteredSessions.length === 0 ? (
                <div className="bg-white rounded-3xl border border-[#e2e8f0] p-12 text-center shadow-sm">
                  ...
                  <p className="text-sm font-bold text-[#112237]">No hay compras en esta categoría</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {filteredSessions.map((session) => {
                    // Obtener todos los productos de la compra para la miniatura
                    const allProducts = session.packages.flatMap((p) => p.items);
                    const previewProducts = allProducts.slice(0, 4);
                    const extraCount = allProducts.length - previewProducts.length;
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#f1f5f9]">
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            <div className="flex items-center gap-1.5 text-xs text-[#475569]">
                              ...
                              <span>Comprado el {formatFullDate(session.createdAt)}</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-3 pt-2 md:pt-0 border-t md:border-t-0 border-[#f1f5f9] shrink-0">
                            <span className="text-xs text-[#64748b] font-semibold flex items-center gap-1">
                              ...
                              <span>
                                ...
                                {session.packages.length === 1 ? "despacho" : "despachos"}
                              </span>
                            </span>
