...
const PasswordInput = React.forwardRef<HTMLInputElement, PasswordInputProps>(
  (
    ...
    const requirements = React.useMemo(() => getPasswordRequirements(stringValue), [stringValue]);
      ...
      <div className="space-y-2 w-full">
        ...
        {showRequirements && stringValue.length > 0 && (
          <div className="p-3 bg-[#f8fafc] border border-[#e2e8f0] rounded-lg space-y-1.5 text-xs mt-2">
            <p className="font-medium text-[#334155] mb-1">Requisitos de la contraseña:</p>
            {requirements.map((req, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <span className={req.met ? 'text-[#10b981] font-medium' : 'text-[#64748b]'}>
                  {req.label}
                </span>
              </div>
            ))}
        ...
        {isMatchError && (
          <p className="text-xs text-[#ef4444] font-medium">Las contraseñas no coinciden</p>
        )}
      </div>
    );
  }
