...
export function detectForbiddenContactInfo(
  rawText: string,
  ...
  if (emailMatch) {
    return {
      hasViolation: true,
      reason:
        "Por políticas de seguridad, no se permiten correos electrónicos en la descripción.",
  ...
  const urlRegex =
    /\b(?:https?:\/\/|www\.|wa\.me\/|t\.me\/|instagram\.com\/|facebook\.com\/)[^\s]+/i;
  const urlMatch = text.match(urlRegex);
  if (urlMatch) {
    return {
      hasViolation: true,
      reason:
        "Por seguridad, no se permiten enlaces ni páginas web externas en la descripción.",
      matchedText: urlMatch[0],
    };
  }

  // 3. Detectar palabras clave explícitas de contacto asociadas a números
  const keywordPhoneRegex =
    /\b(?:wsp|whatsapp|celular|cel|llamar|telefono|tlf|contacto|escribir)\b[\s:=.-]*\+?(?:51)?\s*9\d{2}[\s.-]?\d{3}[\s.-]?\d{3}\b/i;
  const keywordPhoneMatch = text.match(keywordPhoneRegex);
  if (keywordPhoneMatch) {
    return {
      hasViolation: true,
      reason:
        "Por favor no solicites contacto directo ni WhatsApp en la descripción.",
  ...
  const standAloneMobileRegex =
    /(?<![\d\w])(?:\+?51[\s.-]?)?9\d{2}[\s.-]?\d{3}[\s.-]?\d{3}(?![\d\w])/;
  const mobileMatch = text.match(standAloneMobileRegex);
  if (mobileMatch) {
    return {
      hasViolation: true,
      reason:
        "Por tu seguridad y la del comprador, no incluyas números telefónicos en la descripción.",
