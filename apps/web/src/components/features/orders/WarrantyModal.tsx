...
import { AlertCircle, Building2, CheckCircle2, Clock, HelpCircle, Loader2, ShieldCheck, X } from "lucide-react";
...
export const WarrantyModal: React.FC<WarrantyModalProps> = ({
  ...
  const [existingRefunds, setExistingRefunds] = useState<{ id: string; status: string; type: string; items: { order_item_id: string }[] }[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/orders/refund?orderId=${encodeURIComponent(orderId)}`)
      ...
      .catch((err) => console.error("[WarrantyModal] Error fetching refunds:", err));
  }, [isOpen, orderId]);
  ...
  useEffect(() => {
    if (allIds.size > 0 && selectedIds.size === allIds.size && refundType === "partial") {
      setRefundType("full");
      setSelectedIds(new Set());
    }
  ...
  const handleRefundRequest = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        orderId,
        reason: reason.trim() || "Solicitud de reembolso por protección al comprador",
        type: refundType,
      };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
        <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-[#e2e8f0] relative space-y-5">
          <button onClick={onClose} className="absolute top-4 right-4 text-[#94a3b8] hover:text-[#112237]"><X className="w-5 h-5" /></button>
          <div className="text-center space-y-3 pt-4">
            <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto">
              <CheckCircle2 className="w-8 h-8" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-[#112237]">¡Solicitud Enviada!</h3>
              <p className="text-sm text-[#64748b] mt-1">
                Tu solicitud de reembolso para la orden <strong>#{orderCode}</strong> ha sido recibida. Nuestro equipo la revisará y te notificará por correo.
              </p>
            </div>
            <Button onClick={onClose} className="w-full bg-[#f25c05] hover:bg-[#d94d04] text-white font-bold rounded-xl">Entendido</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-[#e2e8f0] relative space-y-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 text-[#94a3b8] hover:text-[#112237]"><X className="w-5 h-5" /></button>
        ...
        <div className="text-center space-y-2">
          ...
          <h3 className="text-lg font-bold text-[#112237]">Protección al Comprador Iubizon</h3>
          <p className={`text-xs font-semibold px-3 py-1 rounded-full inline-block ${isWithinProtection ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>
            {isWithinProtection ? `Dentro del plazo · ${protectionDays - diffDays} días restantes` : "Plazo de protección inicial vencido"}
          </p>
        </div>
        ...
        <div className="bg-[#f8fafc] rounded-2xl p-4 space-y-3 text-xs text-[#475569]">
          ...
          <ul className="space-y-2">
            <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />Producto no coincide con la descripción</li>
            <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />Producto llegó dañado o defectuoso</li>
            <li className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" />Producto no fue entregado</li>
          </ul>
          {productTitle && !hasItems && (
            <p className="text-[11px] pt-1 italic">Producto: <strong>{productTitle}</strong>{sellerName ? ` · Vendedor: ${sellerName}` : ""}</p>
          )}
        </div>
              ...
              <div className="space-y-3">
                ...
                {refundType === "partial" && (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-[#112237]">Selecciona los productos a reembolsar:</span>
                      {availableIds.size > 0 && selectedIds.size < availableIds.size && (
                        <button
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {(() => {
                        ...
                        return Array.from(grouped.entries()).map(([companyName, companyItems]) => (
                          <div key={companyName}>
                            {grouped.size > 1 && (
                            ...
                            {companyItems.map((item) => {
                              const isBlocked = blockedItemIds.has(item.orderItemId);
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <p className="text-xs font-semibold text-[#64748b] truncate">{item.title}</p>
                                      <p className="text-[11px] text-amber-600">
                                        S/ {item.price.toFixed(2)} × {item.quantity} · Reembolso pendiente
                                      </p>
                                    </div>
                                    <span className="text-[11px] font-semibold text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full shrink-0">Pendiente</span>
                                  </div>
                                );
                              }
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    checked={selectedIds.has(item.orderItemId)}
                                    onChange={() => toggleItem(item.orderItemId)}
                                  />
                                  <div className="min-w-0 flex-1">
                                    <p className="text-xs font-semibold text-[#112237] truncate">{item.title}</p>
                    ...
                    {selectedIds.size > 0 && (
                      <div className="flex items-center justify-between bg-[#f8fafc] rounded-xl p-3 border border-[#e2e8f0]">
                        ...
                        <span className="text-sm font-black text-[#f25c05]">S/ {selectedTotal.toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {refundType === "full" && hasItems && !hasAnyRefund && (
                  <div className="bg-[#f8fafc] rounded-xl p-3 border border-[#e2e8f0] text-center">
                    <p className="text-[11px] text-[#64748b]">Se reembolsará el monto total de la orden (incluye envío)</p>
                ...
                {hasAnyRefund && (
                  <div className="bg-amber-50 rounded-xl p-3 border border-amber-200 text-center">
                    <p className="text-[11px] font-semibold text-amber-700">
                      Ya existe una solicitud de reembolso en proceso para esta orden. No puedes solicitar uno nuevo hasta que sea resuelto.
                    </p>
                  </div>
                )}
              </div>
              ...
              <span>
                {loading
                  ...
                  : refundType === "partial" && selectedIds.size > 0
                    ...
                    : "Solicitar Reembolso (Protección " + protectionDays + " Días)"}
              </span>
            </Button>
          </div>
        )}
        ...
        {!isWithinProtection && (
          <Button
            ...
            onClick={() => { onClose(); alert(`Solicitud de mediación enviada para la orden #${orderCode}. Un agente de Iubizon revisará el caso con el vendedor.`); }}
          >
        ...
        <Button variant="outline" className="w-full text-xs font-semibold py-2.5 rounded-xl border-[#cbd5e1] hover:bg-slate-50" onClick={onClose}>Cerrar</Button>
      </div>
    </div>
  );
};
