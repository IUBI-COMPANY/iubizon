...
export async function POST(request: Request) {
  ...
  if (!allowedTypes.includes(file.type)) {
    return NextResponse.json(
      { error: "Tipo de archivo no válido. Sube un video MP4, WEBM o MOV." },
      { status: 400 },
    );
  }
  ...
  if (file.size > 25 * 1024 * 1024) {
    return NextResponse.json(
      { error: "El video excede el límite de 25MB" },
      { status: 400 },
    );
  }
  ...
  if (uploadError) {
    ...
    return NextResponse.json(
      { error: `Error en almacenamiento de video: ${uploadError.message}` },
      { status: 500 },
    );
  }
  ...
  if (productId) {
    try {
      ...
      console.warn(
        "No se pudo actualizar la columna video_url directamente:",
        dbErr,
      );
    }
  }
