        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <FormField name="email" label="Email" required error={errors.email?.message}>
              <Input id="field_email" type="email" placeholder="correo@ejemplo.com" {...register("email")} />
            </FormField>

            <FormField name="password" label="Contraseña" required error={errors.password?.message}>
              <Input id="field_password" type="password" placeholder="••••••••" {...register("password")} />
            </FormField>

            {serverError && <p className="text-xs text-destructive">{serverError}</p>}

            <Button type="submit" className="w-full" disabled={isSubmitting}>
              {isSubmitting ? <><IconLoader2 className="w-4 h-4 mr-2 animate-spin" />Ingresando...</> : "Iniciar Sesión"}
            </Button>
          </form>
        </CardContent>
      </Card>
