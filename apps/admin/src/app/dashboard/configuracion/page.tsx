...
export default function ConfigPage() {
  ...
  if (loading) return <div className="flex justify-center p-8"><IconLoader2 className="h-8 w-8 animate-spin text-primary" /></div>;
            ...
            <CardContent>
              {editing === s.key ? (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
                  <FormField name="json" label="JSON Value" error={errors.json?.message}>
                    <textarea className="w-full h-40 font-mono text-xs p-2 border rounded-md" {...register("json")} />
                  </FormField>
                  <div className="flex gap-2">
                    <Button type="submit" size="sm" disabled={isSubmitting}>
                      {isSubmitting ? <IconLoader2 className="w-3 h-3 mr-1 animate-spin" /> : <IconCheck className="w-3 h-3 mr-1" />}
                      Guardar
                    </Button>
